...
from controller import rulesController
...
class add(RequestHandler):
  def post(self):    
    ...
    print(data)
    sys.stdout.flush()
    if 'rules_code' not in data:
        data['rules_code'] = generateCode()

    if 'object_format' in data:
        data["script_format"] = rulesController.converter(data["object_format"])

    insert = rulesController.add(data)    
...
class list(RequestHandler):
  def post(self):    
    ...
    query = data    
    result = rulesController.find(query)
  ...
  def post(self):    
    ...
    if "id" in query :
        try:
            ...
            del query["id"]
    result = rulesController.findOne(query)    
    if not result['status']:
        response = {"status":False, "message":"Data Not Found",'data':json.loads(self.request.body)}               
    else:
        response = {"status":True, 'message':'Success','data':result['data']}
...
class update(RequestHandler):
  def post(self):        
    ...
    if 'rules_code' in data:
        if checkActionCode(data['rules_code'],query['_id']):
    ...
    if not result['status']:
        ...
        update = rulesController.update(query,data)
...
class delete(RequestHandler):
  def post(self):        
    ...
    if not result['status']:
        ...
        delete = rulesController.delete(query)
...
def checkActionCode(code,execpt=""):
    if execpt:
        query = {"rules_code":code,"_id":{ '$ne' : execpt } }
        result = rulesController.findOne(query)
    else:
        query = {"rules_code":code}
        result = rulesController.findOne(query)
